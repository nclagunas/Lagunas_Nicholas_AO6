...
[https://github.com/nclagunas/Lagunas_Nicholas_AO6.git](https://github.com/nclagunas/Lagunas_Nicholas_AO6.git)
...
for(i in 1:length(player_info[[1]])){
  
  
  #since the number of heights = number of players, iterate through both simultaneously 
  player_info[[i]] = append(player_info[[i]],list(height = heights[i]))
...
for(i in player_info){
  ...
  #take the index of the first white space in name
  white_space = gregexpr(" ",i$name)[[1]][[1]] 
  
  #take the initials of the player by taking the first letter of their name and the first letter after a white space
  intials = append(intials,gsub(" ","", paste(substr(i$name, 0, 1),substr(i$name, white_space,white_space+1))))

  
  points_per_game = append(points_per_game, i$points/i$games_played) 
  
  
  
}

#assign the initials to the correct player
names(points_per_game) = intials
...
points_per_game_rounded = lapply(points_per_game, round,2)
...
mat_rand = apply(mat_rand,1:2,signif,1)
...
#traverse through all the months
for(i in 3:length(max_temps)){
  
  #take the means at each month
  means = append(means,mean(max_temps[[i]]))
}
...
data_CO = max_temps[max_temps$state == "CO",]
data_OH = max_temps[max_temps$state == "OH",]
data_VA = max_temps[max_temps$state == "VA",]
...
for(i in 1:length(data_CO[[1]])){
  
  #take the values for each of the months
  months = data_CO[i,3:14]
  
  #take the median and add to the vect
  co_medians = append(co_medians,median(as.numeric(months)))
  
}

#defines the years of each of the medians
names_years = data_CO[[2]]

#sets the years to their corresponding median
co_medians = set_names(co_medians,names_years)

#returns max and min years
max_med = names(which.max(co_medians))
min_med = names(which.min(co_medians))




paste0("The maximum median annual temperature of ",co_medians[max_med], " occurred in ", max_med," and the minimum median annual temperature of ", co_medians[min_med], " occurred in ", min_med)
...
sum_of_digits = function(digit = NA){
  
  #convert number to characters
  digit_str = as.character(digit)
  
  #store the first digit
  first_digit = substr(digit_str,0,1)
  
  #store the rest of the digits
  rest_of = substr(digit_str,unlist(gregexpr(first_digit,digit_str))+1,nchar(digit_str))
  
  #convert the char to a num
  first_digit = as.numeric(first_digit)
  
  #this is our baseline
  #if there is no more digits after the first one then return
  if(is.na(as.numeric(rest_of))){
    
    return(first_digit)
  
    }
  
  #convert to num for addition
  rest_of = as.numeric(rest_of)
   
  
  #add the first digit plus the next digit recursively
  return(first_digit+sum_of_digits(rest_of))

  
}


paste0(sum_of_digits(123))
paste0(sum_of_digits(935068))
